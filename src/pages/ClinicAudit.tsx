        ...
        <div className="container mx-auto px-4 lg:px-8 max-w-5xl">
          <div className="grid md:grid-cols-2 gap-8">
            {assessments.map((audit, i) => (
              <motion.div key={audit.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="bg-card rounded-2xl p-8 lg:p-10 shadow-card">
                <div className="w-14 h-14 rounded-2xl bg-secondary flex items-center justify-center"><audit.icon className="h-7 w-7 text-primary" /></div>
                ...
                <div className="mt-6">
                  ...
                  <ul className="space-y-2">
                    {audit.includes.map((item) => (
                      <li key={item} className="flex items-center gap-2 text-sm text-muted-foreground"><ClipboardCheck className="h-4 w-4 text-accent flex-shrink-0" />{item}</li>
                    ))}
                  </ul>
                </div>
                <Button variant="hero-outline" size="default" asChild className="mt-8">
                  <Link to="/strategic-assessment">Learn About This Assessment <ArrowRight className="ml-2 h-4 w-4" /></Link>
                </Button>
              </motion.div>
            ))}
          </div>
        ...
        <div className="container mx-auto px-4 lg:px-8 max-w-3xl text-center">
          ...
          <div className="mt-12 grid sm:grid-cols-3 gap-8">
            {[
              ...
              <div key={s.step}>
                <div className="w-12 h-12 rounded-full bg-primary text-primary-foreground flex items-center justify-center font-display text-lg font-bold mx-auto">{s.step}</div>
