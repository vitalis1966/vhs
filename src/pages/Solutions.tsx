...
const practiceTypes = [
  "Family medicine practices", "Specialty practices", "Multidisciplinary practices",
  "Dental practices", "Veterinary practices", "Non-hospital surgical facilities",
];
      ...
      <section className="pt-32 pb-20 lg:pt-40 lg:pb-28 bg-gradient-hero">
        <div className="container mx-auto px-4 lg:px-8 max-w-4xl">
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mt-8 text-lg text-muted-foreground leading-relaxed max-w-3xl">
            Based in Calgary, Vitalis Health Strategies supports practitioners and healthcare organizations across Canada through every stage of growth — from initial concept through long-term strategic advisory. Our <Link to="/about" className="text-primary hover:text-foreground underline underline-offset-2 transition-colors">clinician-led team</Link> brings operational experience to every engagement. Start with a <Link to="/strategic-assessment" className="text-primary hover:text-foreground underline underline-offset-2 transition-colors">strategic assessment</Link> to understand where your practice stands.
          </motion.p>
        </div>
      </section>
      ...
      <section className="py-20 lg:py-28 bg-background">
        <div className="container mx-auto px-4 lg:px-8 max-w-6xl">
          <div className="text-center mb-16">
            ...
            <p className="mt-4 text-muted-foreground text-lg max-w-2xl mx-auto">Vitalis provides tailored solutions whether you're planning a new practice or looking for an objective view of an existing one.</p>
          </div>
          <div className="grid lg:grid-cols-2 gap-8">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-card rounded-2xl p-8 lg:p-10 shadow-card border border-border/40 hover:shadow-elevated transition-shadow duration-300">
        ...
        <div className="container mx-auto px-4 lg:px-8 max-w-6xl">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {capabilities.map((cap, i) => (
              <motion.div key={cap.title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="bg-card rounded-2xl p-6 shadow-soft border border-border/40 hover:shadow-card transition-shadow duration-300">
                <div className="w-11 h-11 rounded-xl bg-secondary flex items-center justify-center mb-4"><cap.icon className="h-5 w-5 text-primary" /></div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="space-y-3">
              {practiceTypes.map((type, i) => (
                <motion.div key={type} initial={{ opacity: 0, x: 15 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }} className="bg-card rounded-xl px-6 py-4 shadow-soft border border-border/40 flex items-center gap-3">
                  <BarChart3 className="h-4 w-4 text-accent flex-shrink-0" /><span className="text-sm font-medium text-foreground">{type}</span>
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <div className="container mx-auto px-4 lg:px-8 max-w-6xl">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {organizations.map((org, i) => (
              <motion.div key={org.title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="bg-card rounded-2xl p-7 shadow-soft border border-border/40">
                <div className="w-12 h-12 rounded-xl bg-secondary flex items-center justify-center mb-5"><org.icon className="h-6 w-6 text-primary" /></div>
      ...
      <section className="py-20 lg:py-28 bg-background">
        <div className="container mx-auto px-4 lg:px-8 max-w-4xl text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <h2 className="font-display text-3xl lg:text-4xl font-bold text-foreground mb-6">Full-lifecycle advisory partnership.</h2>
            <p className="text-muted-foreground text-lg leading-relaxed mb-8 max-w-2xl mx-auto">Many organizations retain Vitalis as a long-term strategic advisor — providing ongoing guidance as their organization grows, evolves, and navigates new challenges.</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Button variant="hero" size="xl" asChild><Link to="/strategic-assessment">Start Your Strategic Assessment <ArrowRight className="ml-2 h-5 w-5" /></Link></Button>
            </div>
          </motion.div>
        </div>
      </section>
