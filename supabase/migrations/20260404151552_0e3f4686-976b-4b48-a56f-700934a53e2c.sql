...
BEGIN
  ...
  -- If the token is valid but the report row doesn't exist yet (async generation),
  -- return a 'pending' status so the frontend can poll instead of showing an error.
  ...
  -- Report is ready — NOW track the access
  BEGIN
